...
E realizar a requisição com um id em branco
    [Documentation]    Realiza uma requisição sem informar um id
...
Então o sistema deve retornar um json contendo todos os produtos cadastrados
    [Documentation]    Verifica se a quantidade de produtos retornados está correta
    ${HEADERS}    Create Dictionary    Authorization=${TOKEN}
    ${RESPONSE}    GET On Session    alias=serveRest    url=/produtos/    headers=${HEADERS}
    Log    Resposta Retornada: ${\n}${RESPONSE.text}
    Should Be Equal As Integers    ${RESPONSE.status_code}    200
    Dictionary Should Contain Item    ${RESPONSE.json()}    quantidade    ${2}
